...
class SWAT_Run():
    def __init__(self, SWATtxtinoutFolder, pop_size_project):
        ...
        # Subbasin/HRU
        dtype_dict = {'HRU_GIS': str}
        HRU_df            = pd.read_excel(f'{self.swat_nsga_in}\\FullHRU_Landuse_Area.xlsx', dtype=dtype_dict)
        ...
        self.cal_vars_list = ['Streamflow', 'ET']
    ...
    def SWAT_model_execution(self, SWATParas_Sampling):
        ...
        # Step 4: Modify SWAT Project Parameter Files
        print('Step 4: Modify SWAT Project Parameter Files'.center(self.comments_len, '='))
        ...
        if mod_flag:
            ...
            if self.para_fix_mode:
                self.modify_SWAT_para_multiprocessing(1, f'{self.swat_model}\\Compro_sol', para_sampling_dict)
            elif self.cal_val_state == 'Validation':
                ...
                with Pool(processes=self.cpu_worker_num) as p:
                    p.starmap(self.modify_SWAT_para_multiprocessing, process_args)
            else:
        ...
        # Step 5: SWAT Model Execution
        print('Step 5: SWAT Model Execution'.center(self.comments_len, '='))
        run_flag = True
        if run_flag:
        ...
        # Step 6: Read SWAT Model Simulation Data
        read_sim_start_time = time.time()
        print('Step 6: Read SWAT Model Simulation Data'.center(self.comments_len, '='))
        ...
        # Step 7: Calculate Objective Functions
        cal_obj_start_time = time.time()
        print('Step 7: Calculate Objective Functions'.center(self.comments_len, '='))
        # One Variables
        if self.cal_vars_list == ['Streamflow']:
        ...
        # Three Variables
            ...
            if self.objec_mode == 'Area_Weighted_Avg':
                ## Area weighted average LAI simulation with Multiprocessing
                print('Area weighted average LAI simulation using multiprocessing:')
                process_args = [(LAI_pop_idx, self.HRU_ID_Veg, self.HRU_veg_area_dict) for LAI_pop_idx in LAI_ET_HRU_data_dict_PopList_sort]
                print('process_args:', len(process_args))
                with Pool(processes=self.cpu_worker_num) as p:
                    sim_LAI_avg = p.starmap(self.Veg_sim_area_weighted_avg, process_args)
                LAI_data_dict_PopList_sort_w = [sim_lai_idx for sim_lai_idx in sorted(sim_LAI_avg, key=lambda x: x[0])]
                ...
                ET_data_dict_PopList_sort = None
                if self.spatial_unit == 'HRU':
                    ET_data_dict_PopList_sort = LAI_ET_HRU_data_dict_PopList_sort
                ## Area weighted average LAI simulation with Multiprocessing
                print('Area weighted average ET simulation using multiprocessing:')
                process_args = [ET_pop_idx for ET_pop_idx in ET_data_dict_PopList_sort]
                print('process_args:', len(process_args))
                with Pool(processes=self.cpu_worker_num) as p:
                    sim_ET_avg = p.map(self.RS_LAI_ET_sim_area_weighted_avg, process_args)
                ET_data_dict_PopList_sort_w = [sim_et_idx for sim_et_idx in sorted(sim_ET_avg, key=lambda x: x[0])]
                print('ET_data_dict_PopList_sort_w:', len(ET_data_dict_PopList_sort_w))
                #
                for ET_sim_pop_idx in ET_data_dict_PopList_sort_w:
    ...
    def read_swat_para(self):
        swat_para = np.loadtxt(fname=os.path.join(self.swat_nsga_in, 'SWAT_Par.def'), dtype=str, skiprows=1, usecols=(0, 1, 2)).tolist()
